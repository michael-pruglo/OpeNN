...
void NetworkTrainer::set_network(NeuralNetwork* network)
{
    nn = network;
}

void NetworkTrainer::set_hyper_parameters(TrainingHyperParameters hyper_params)
{
    hyper_parameters = hyper_params;
}

void NetworkTrainer::set_training_data(const std::vector<TrainingSample>* const tr_data)
{
    training_data = tr_data;
}

void NetworkTrainer::set_validation_data(const std::vector<TrainingSample>* const val_data)
{
    validation_data = val_data;
}

void NetworkTrainer::set_test_data(const std::vector<TrainingSample>* const tst_data)
{
    test_data = tst_data;
}

void NetworkTrainer::train(bool verbose)
{
    assert(nn);
...
void NetworkTrainer::epoch_full_gradient_descent(TrainingConstIt first, TrainingConstIt last)
{
    Gradient total_grad;
...
void NetworkTrainer::epoch_stochastic_gradient_descent(TrainingConstIt first, TrainingConstIt last, size_t batch_size)
{
...
void NetworkTrainer::epoch_online_learning(TrainingConstIt first, TrainingConstIt last)
{
    epoch_stochastic_gradient_descent(first, last, 1); //unless we can get a significant speed up
}

Gradient NetworkTrainer::backprop(const TrainingSample& sample)
{
    throw "not implemented";
}

core::float_t NetworkTrainer::eval_average_cost()
{
...
namespace
{
    bool is_classification_problem(const std::vector<TrainingSample>* const test_data)
    {
...
size_t NetworkTrainer::eval_correct_guesses()
{
